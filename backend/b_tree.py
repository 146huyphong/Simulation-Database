...
class BTreeNode:
    def __init__(self, leaf=False):
        self.leaf = leaf
        self.keys = []
        self.offsets = [] # Đã chuyển thành mảng 2 chiều: [[offset1], [off2, off3]]
        self.children = []
...
class BTree:
    ...
    def search(self, key, node=None):
        ...
        if i < len(node.keys) and key == node.keys[i]:
            return node.offsets[i] # Trả về list các offset
        if node.leaf:
            return None
        return self.search(key, node.children[i])
    
    def _add_offset_if_exists(self, node, key, offset):
        """Hỗ trợ chèn trùng tên: Nếu key đã có, chỉ việc nối thêm offset"""
        i = 0
        while i < len(node.keys) and key > node.keys[i]:
            i += 1
        if i < len(node.keys) and key == node.keys[i]:
    ...
    def insert(self, key, offset):
        # Kiểm tra nếu key đã tồn tại thì chỉ thêm offset
    ...
    def _insert_non_full(self, node, key, offset):
        ...
        if node.leaf:
            ...
            node.offsets.insert(i, [offset]) # Bọc offset trong một list
        else:
    ...
    def delete(self, key, offset_to_remove=None):
        if not self.root.keys: return False
    ...
    def _delete_recursive(self, node, key, offset_to_remove):
        ...
        if i < len(node.keys) and key == node.keys[i]:
            # Chỉ xóa offset được yêu cầu. Nếu key vẫn còn offset khác (trùng tên), không xóa key.
            if offset_to_remove is not None and offset_to_remove in node.offsets[i]:
                ...
                if len(node.offsets[i]) > 0:
                    return True # Vẫn còn người trùng tên, giữ nguyên Node
    ...
    def _fix_underflow(self, parent, index):
        ...
        if index > 0 and len(parent.children[index - 1].keys) > self.min_keys:
            ...
            if not child.leaf: child.children.insert(0, left_sibling.children.pop(-1))
            return
        if index < len(parent.children) - 1 and len(parent.children[index + 1].keys) > self.min_keys:
            ...
            if not child.leaf: child.children.append(right_sibling.children.pop(0))
            return 
        if index > 0: self._merge_nodes(parent, index - 1)
        else: self._merge_nodes(parent, index)
    ...
    def _merge_nodes(self, parent, index):  
        ...
        if not left_child.leaf: left_child.children.extend(right_child.children)
        parent.children.pop(index + 1)  

    def get_tree_state(self):
        if not self.root.keys: return {}
